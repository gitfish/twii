...
import { StateManager } from "@twii/core/lib/model/StateManager";
import * as PathUtils from "@twii/core/lib/PathUtils";
...
class ListingModel extends StateManager implements IListingModel {
    ...
    validate(approvalStatus : ListingApprovalStatus) {
        ...
        if(isBlank(this.title)) {
            this.validationErrors.push({ prop: "title", propTitle: "Title", message: "Title is required"});
        }
        if(isBlank(this.security_marking)) {
            this.validationErrors.push({ prop: "security_marking", propTitle: "Security", message: "Security is required"});
        }
        if(this.id) {
            // launch url required any time trying to save after creation
            if(isBlank(this.launch_url)) {
                this.validationErrors.push({ prop: "launch_url", propTitle: "Launch URL", message: "Launch URL is required" });
            }
        }
        ...
        if(approvalStatus !== ListingApprovalStatus.IN_PROGRESS) {
            if(isBlank(this.description_short)) {
                this.validationErrors.push({ prop: "description_short", propTitle: "Short Description", message: "Short Description is required" });
            }
            if(isBlank(this.description)) {
                this.validationErrors.push({ prop: "description", propTitle: "Description", message: "Description is required"});
            }
        }
