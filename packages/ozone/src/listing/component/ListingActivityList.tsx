...
import { timestampIO } from "@twii/core/lib/MomentDataUtils";
    ...
    private _onRenderActivityHeader = (props : IGroupDividerProps) => {
        ...
        return (
            <div className={this.props.classNames.itemsGroupHeader}>
                <strong>{activity.action}</strong> - {activity.author.display_name} - {timestampIO(activity.activity_date)}
            </div>
        );
    }
    render() {
        ...
        this.props.activityList.itemsView.forEach((activity, idx) => {
            groups.push({
                key: String(idx),
                name: `${activity.action} - ${activity.author.display_name} - ${timestampIO(activity.activity_date)}`,
