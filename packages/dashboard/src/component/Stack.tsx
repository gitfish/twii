...
class StackTabIcon extends React.Component<IStackWindowProps, any> {
    render() {
        ...
        if(icon.name || icon.text || icon.url || icon.component) {
            const classNames = getClassNames(getStyles(null, this.props.styles), this.props.className);
            return (
                <div className={classNames.tabIconContainer}>
        ...
        if(stack.addApp) {
            ...
            return (
                <button type="button"
                        ...
                        onClick={this._onClick}
                        style={{ width: stack.headerHeight }}>
                    <Icon className={classNames.addActionIcon} iconName="Add" />
                </button>
            );
        }
        return null;
...
class StackTabBar extends React.Component<IStackProps, any> {
    ...
    render() {
        ...
        const tabs = this.props.stack.windows.map((w, idx) => {
            return <StackTab key={w.id} {...this.props} window={w} first={idx === 0} last={idx === this.props.stack.windowCount - 1} />;
        });
        return (
