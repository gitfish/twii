...
interface ISearchRequestSearchStringSupplierOptions {
    request: ISearchRequest;
    schema: ISearchSchema;
    defaultOp?: SearchGroupOperator;
}

class SearchRequestSearchStringSupplier implements ISupplier<string> {
    private _defaultOp : SearchGroupOperator;
    request : ISearchRequest;
    schema: ISearchSchema;
    constructor(opts?: ISearchRequestSearchStringSupplierOptions) {
    ...
    get searchString() {
        ...
        if(this.schema && ((this.request.fields && this.request.fields.length > 0) || (this.request.groups && this.request.groups.length > 0))) {
            const supplier = new SearchGroupSearchStringSupplier({
                group: this.request,
                schema: this.schema
            });
...
export { SearchRequestSearchStringSupplier, ISearchRequestSearchStringSupplierOptions }