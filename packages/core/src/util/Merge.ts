...
const deepEquality : IEquality = (value : any, other : any) => {
    if(value === other) {
        return true;
    }
    if(isArray(value)) {
...
const defaultSimpleMergeStrategy : IMergeStrategy = (l : any, r : any) => {
    if(l === r) {
        return l;
    }
    return [l, r];
...
const stringConcatSimpleMergeStrategy = (separator : string = ",") : IMergeStrategy => {
    return (l : any, r : any) => {
        if(isString(l)) {
...
const MergeDefaults : IMergeOptions = {
    eq: defaultEquality,
...
const mergeDedup = (value : any, other : any, opts?: IMergeOptions) : any => {
...
const mergeAllDedup = (values : any[], opts?: IMergeOptions) : any => {
    return mergeAllDedupImmediate(values, Object.assign({}, MergeDefaults, opts));
};

const dedupArray = (value : any[], eq: IEquality = defaultEquality) : any[] => {
    if(value) {
        const r = [];
        value.forEach(e => {
