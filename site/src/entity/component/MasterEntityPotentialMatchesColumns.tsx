...
const getDateOfBirthText = (item: IMasterEntityPotentialMatch) => {
    return DateUtils.dataToOutputText(item.Date_Of_Birth);
};

const getStartTimestampText = (item: IMasterEntityPotentialMatch) => {
    return DateUtils.dataTimestampToOutputText(item.Cdl_Strt_Tmstmp);
};

const getEndTimestampText = (item: IMasterEntityPotentialMatch) => {
    return DateUtils.dataTimestampToOutputText(item.Cdl_End_Tmstmp);
};

const getCredentialText = (item : IMasterEntityPotentialMatch) : string => {
...
const DateOfBirth: IColumn = {
    ...
    isResizable: true,
    data: {
        getText(item : IMasterEntityPotentialMatch) {
            return getDateOfBirthText(item);
        }
    },
    onRender: (item: IMasterEntityPotentialMatch) => {
        return getDateOfBirthText(item);
    }
};
...
const MasterEntityPotentialMatchesColumns : IColumn[] = [
    ClientID,
    SourceEntityName,
    DateOfBirth,
    Gender,
...
export {
    MasterEntityPotentialMatchesColumns as default,
    MasterEntityPotentialMatchesColumns,
    MatchScorePercentage,
    Credential,
    StartTimestamp,
    EndTimestamp,
