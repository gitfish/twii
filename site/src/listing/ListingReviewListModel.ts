import { action, observable, computed } from "mobx";
import IListing from "./IListing";
import { IListingReview } from "./IListingReview";
import { IListingReviewListModel } from "./IListingReviewListModel";
import ListModel from "common/ListModel";
import { IListingService, IListingReviewListRequest } from "./IListingService";
import ListingServiceContext from "./ListingServiceContext";
import { IListingListCounts } from "./IListingListCounts";
import { SyncModel } from "common/SyncModel";
import { isNotBlank } from "util/String";
import { IListingModel } from "./IListingModel";
import { IListingReviewModel } from "./IListingReviewModel";
import { ListingReviewModel } from "./ListingReviewModel";
import { toPromise } from "common/SyncUtils";

class ListingReviewListModel extends ListModel<IListingReview> implements IListingReviewListModel {
    private _listingService : IListingService;
    
    @observable _newReview : ListingReviewModel;

    @observable private _listing : IListingModel;

    constructor(listing : IListingModel) {
        super();
        this._listing = listing;
    }
    
    get listingService() {
        return this._listingService || ListingServiceContext.value;
    }
    set listingService(value : IListingService) {
        this._listingService = value;
    }

    @computed
    get listing() {
        return this._listing;
    }

    @action
    private _refreshDone = (reviews : IListingReview[]) => {
        this.setItems(reviews);
    }

    @action
    private _refreshError = (error : any) => {
        this.clearItems();
        this.sync.syncError(error);
    }

    @action
    refresh() {
        this.sync.syncStart();
        this.cancelEdit();
        return this.listingService.getListingReviews({ listingId: this.listing.id }).then(this._refreshDone).catch(this._refreshError);
    }

    @action
    load() : Promise<any> {
        if(!this.sync.syncing && (!this.sync.hasSynced || this.sync.error)) {
            return this.refresh();
        }
        return toPromise(this.sync);
    }

    @computed
    get newReview() {
        return this._newReview;
    }

    @action
    add() : void {
        this._newReview = new ListingReviewModel(this.listing);
    }

    @action
    cancelEdit() : void {
        this._newReview = undefined;
    }
}

export { ListingReviewListModel as default, ListingReviewListModel }